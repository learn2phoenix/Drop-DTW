...
def framewise_assignment_2_sted_anno(args, framewise_assignment, sample, dataset):
    """convert framewise assignments to start and end times"""
    # get text annotations from json if youcook2
    if args.dataset == "YouCook2":
        json_path = "/mnt/DATA/Datasets/YouCook2/youcookii_annotations_trainval.json"
        data = json.load(open(json_path, "r"))["database"]
    # save video level info
    video_anno = dict()
    if args.dataset == "YouCook2":
        video_anno["video_id"] = sample["name"].split("_", 1)[-1]
        video_anno["description"] = sample["name"].split("_", 1)[0]
    else:
        video_anno["video_id"] = sample["name"]
        video_anno["description"] = sample["cls_name"]

    video_anno["duration"] = framewise_assignment.shape[0]
    video_anno["url"] = ["https://youtu.be/" + video_anno["video_id"]]

    video_anno["annotations"] = []
    # save Drop-DTW input/output info
    print(video_anno)
    # input("check anno")
    ids = np.unique(framewise_assignment)
    print(ids)
    # input("check ids")
    for idx in ids:
        if idx != -1:
            anno_sample = dict()
            st_ed = np.where(framewise_assignment == idx)[0]
            st = int(st_ed[0])
            ed = int(st_ed[-1])
            idx = int(idx)
            print(framewise_assignment)
            print(idx + 1, st, ed)
            input("check st ed")
            seg_features = sample["frame_features"][st : ed + 1, :]
            anno_sample["cls_id"] = int(sample["step_ids"][idx].detach().cpu().numpy())
            if args.dataset == "YouCook2":
                name = sample["name"].split("_", 1)[-1]
                anno_sample["txt_annotation"] = data[name]["annotations"][sample["step_ids"][idx]]["sentence"]
            elif args.dataset == "crosstask":
                anno_sample["txt_annotation"] = dataset.get_step_description(sample["step_ids"][idx])
            else:
                anno_sample["txt_annotation"] = sample["steps"][idx]

            anno_sample["start_time"] = st
            anno_sample["end_time"] = ed
            anno_sample["txt_multimodal_embedding"] = sample["step_features"][idx].detach().cpu().numpy().tolist()
            anno_sample["seg_embedding"] = seg_features.detach().cpu().numpy().tolist()
            anno_sample["object_class"] = None
            anno_sample["object_attribute"] = None
            anno_sample["area"] = None
            anno_sample["bbox"] = None
            anno_sample["obj_embedding"] = None
            # save step wise info
            video_anno["annotations"].append(anno_sample)
...
def framewise_eval(dataset, model, keep_p, gamma=1, config=None):
    ...
    for i, sample in enumerate(tqdm(dataset)):
        if sample["num_steps"] < 1:
            continue
        if sample["cls_name"] != "creme-fraiche-mashed-potatoes":
            continue
        ...
        print(step_features.shape)
        print(sim.shape)
        # input("check features and sims")
        ...
        print(optimal_assignment.shape)
        # input("check assignment")

        # save framewise assignments
        video_anno = framewise_assignment_2_sted_anno(config, optimal_assignment, sample, dataset)
        data_anno.append(video_anno)
        ...
        """
        accuracy["simple"] += framewise_accuracy(simple_assignment, sample, use_unlabeled=config.use_unlabeled)
        accuracy["dp"] += framewise_accuracy(optimal_assignment, sample, use_unlabeled=config.use_unlabeled)
        iou["simple"] += IoU(simple_assignment, sample)
        iou["dp"] += IoU(optimal_assignment, sample)
        iou_drop_dtw[sample["name"]] = IoU(optimal_assignment, sample)
        """
    # order results from best to worse
    # iou_drop_dtw = dict(sorted(iou_drop_dtw.items(), key=lambda item: item[1], reverse=True))
    # save ordered youcook2 results
    """
    with open("ME_ordered_results_YC2_model.json", "w") as outfile:
        json.dump(iou_drop_dtw, outfile)
    """
    # save drop_dtw input/output
    with open("ME_input_output_new.json", "w") as outfile2:
        json.dump(data_anno, outfile2)

    num_samples = len(dataset)
    return [v / num_samples for v in [accuracy["simple"], accuracy["dp"], iou["simple"], iou["dp"]]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="ARA", help="dataset")
    parser.add_argument(
        "--name", type=str, default="", help="model for evaluation, if nothing is given, evaluate pretrained features"
    )
    parser.add_argument("--distance", type=str, default="inner", help="distance type")
    parser.add_argument(
        "--dp_algo", type=str, default="DropDTW", choices=["DropDTW", "OTAM", "DTW", "NW", "LCSS"], help="distance type"
    )
    parser.add_argument("--drop_cost", type=str, default="logit", help="Whather do drop in drop-dtw")
    parser.add_argument(
        "--keep_percentile", type=float, default=0.3, help="If drop_cost is logits, the percentile to set the drop to"
    )
    parser.add_argument(
        "--use_unlabeled",
        type=bool,
    ...
    if args.name:
        gamma = 30
        model = EmbeddingsMapping(d=512, learnable_drop=(args.drop_cost == "learn"), video_layers=2, text_layers=0)
    ...
    accuracy_std, iou_std, accuracy_dtw, iou_dtw, recall = compute_all_metrics(dataset, model, gamma, args)
